...
class PPO:
    def __init__(self, state_dim, hidden_dim, action_dim, actor_lr, critic_lr,
                 lmbda, epochs, eps, gamma, minibatch_size):
    ...
    def offline_train(self, states, actions, returns):
        ...
        for _ in range(self.epochs):
            for batch_states, batch_actions, batch_returns in dataset:
                ...
                # print(self.critic(batch_states).shape)
                advantage = td_target - tf.reshape(self.critic(batch_states),(-1))
                ...
                with tf.GradientTape() as tape_actor, tf.GradientTape() as tape_critic:
                    # 计算 log_probs
                    log_probs = tf.math.log(self.actor(batch_states)+1e-10)
